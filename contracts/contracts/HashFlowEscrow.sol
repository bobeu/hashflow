...
contract HashFlowEscrow is IHashFlowEscrow, ReentrancyGuard, Ownable {
    ...
    function createEscrowWithAuth(
        ...
        return _createEscrow(_msgSender(), _worker, _amount, _taxRateBP, _taxRecipient);
    }
    ...
    function _checkVerification(address _worker) internal view {
        if (zkVerifier != address(0)) {
            if(useDemo) {
                ...
                (bool isHuman, uint8 level) = IHashkeyKyc(zkVerifier).checkHuman(_worker);
                require(isHuman && level > 0, "Not verified");
            }
        }
    }
}
